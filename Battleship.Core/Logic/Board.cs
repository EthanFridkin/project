...
namespace Battleship.Core.Logic
{
    ...
    public class Board
    {
        ...
        {
            // בודק שההנחה חוקית — בגבולות ואין חפיפה
            ...
            // סומן כל תא שהצוללת מכסה לפי כיוון ואורך
            for (int i = 0; i < sub.Length; i++)
            {
                int r = row + (sub.Orientation == Orientation.Vertical ? i : 0);
                int c = col + (sub.Orientation == Orientation.Horizontal ? i : 0);
            ...
            // מוסיף את הצוללת לרשימה ומאפס את מונה הפגיעות שלה
            _submarines.Add(sub);
        ...
        {
            // מחלץ שורה ועמודה מהמיקום
            int row = loc.GetRow();
            int col = loc.GetColumn();

            // בודק שהירייה בגבולות הלוח
            if (row < 0 || row >= Size || col < 0 || col >= Size)
                throw new ArgumentOutOfRangeException("מיקום הירייה מחוץ לגבולות הלוח.");

            // בודק שלא ירו כבר על תא זה
            if (_cells[row, col].WasShot)
                throw new InvalidOperationException("כבר ירו על תא זה.");

            // מבצע את הירייה על התא — מחזיר האם פגע
            bool isHit = _cells[row, col].ReceiveShot();

            if (!isHit)
                return ShotResult.Miss;

            // פגיעה — מוצא את הצוללת ומעדכן מונה פגיעות
            int subId = _cells[row, col].SubmarineId;
            _hitCount[subId]++;

            Submarine sub = _submarines.Find(s => s.SubmarineId == subId);       /* פגיעה בצוללת חילוץ הID שלה*/

            // בודק אם מספר הפגיעות הגיע לאורך הצוללת — אם כן הוטבעה
            if (_hitCount[subId] >= sub.Length)
            {
                sub.IsDestroyed = true;       /* מונה פגיעות שבודק האם אכן הצוללת הוטבעה*/
        ...
        //בדיקה האם כל הצוללות הושמדו — אם כן המשחק נגמר
