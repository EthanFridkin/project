...
namespace Project
{
    public partial class GamePage : ContentPage
    {
        ...
        private void BuildArea(AbsoluteLayout area, bool isPlayer)
        {
            ...
            // אם מצב Gemini — שנה צבעים ללוח היריב
        ...
        private async void StartGame_Click(object sender, EventArgs e)
        {
            ...
            if (_isMulti)
            {
                ...
                UpdateStatusLabel(myTurn ? "תורך לירות" : $"תור {_opponentName}", myTurn ? "#009ADA" : "#AA3333");
                StartPolling();
            }
            else
            {
        ...
        /// <summary>
        /// תור Gemini — שולח את מצב הלוח ומקבל ירייה.
        /// אם Gemini נכשל — חוזר לאלגוריתם הרגיל כגיבוי.
        /// </summary>
        private async Task GeminiTurnAsync()
        {
            var playerBoard = _game.GetOpponentBoardPublic(_aiPlayerModel);

            // בניית מצב הלוח כ-int[,]
            ...
            for (int r = 0; r < 10; r++)
            {
                for (int c = 0; c < 10; c++)
                {
                    ...
                    if (!cell.WasShot)
                        boardState[r, c] = 0; // לא ירו
                    else if (!cell.IsHit)
                        boardState[r, c] = 1; // פספוס
                    else
                    {
                        // בדוק אם הצוללת טבועה
                        int subId = cell.SubmarineId;
                        var subCell = playerBoard.GetCell(r, c);
                        boardState[r, c] = 2; // פגיעה (נעדכן ל-3 אם טבועה)
                    }
                }
            }
            ...
            // אם Gemini נכשל או החזיר תא שכבר ירו עליו — גיבוי לאלגוריתם
